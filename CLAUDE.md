...
Detalhe do runtime: o `@wasmer/sdk` é carregado via **import dinâmico do CDN** (`SDK_URL` em `src/runner/wasmerRunner.ts`, pinado em 0.10.0), não como dependência npm — evita atrito do bundler com o worker do SDK. Execução é preguiçosa (baixa o clang só no 1º Executar). **Requer contexto seguro (HTTPS ou localhost)** além dos headers COOP/COEP — `SharedArrayBuffer` não funciona em http:// de domínio/IP (falha vira "erro de CORS"). Deploy usa o perfil `https` do compose (Caddy + Let's Encrypt, var `DOMAIN`); ver README → Deploy.

Publicado em https://github.com/TheKoakuma/blockly-c-ide (público). Remote `origin` em HTTPS (o `gh` está com protocolo SSH mas faltava host key na máquina).

Blocos disponíveis (ver docs/BLOCKS.md): além do MVP (main, variáveis, printf/scanf, if/else, for, while, operadores, literais), há **funções** (com tipo de retorno + return + chamada), **vetores**, **matrizes**, **structs**, **ponteiros** (`&`/`*`) e **memória dinâmica** (malloc/calloc/free). O gerador inclui headers sob demanda (sempre `<stdio.h>`; `<stdlib.h>` quando há alocação dinâmica). Structs/funções são blocos de topo (como o main) — coloque-os acima do main (ordem de geração segue a posição vertical). Nota: `field_multilinetext` NÃO está registrado nesta build do Blockly — por isso campos de struct usam `field_input` separado por `;`. Em DEV, `window.__cgen` (gerador) e `window.__runner` (CRunner) também ficam expostos para depuração.

Nota de verificação: mutações programáticas via `serialization.workspaces.load`/`moveBy` no console NÃO disparam o change listener (logo o painel React não regenera), mas geração e execução podem ser testadas direto por `window.__cgen.workspaceToCode(window.__ws)` e `window.__runner.run(code, {})`. Gestos reais do usuário disparam normalmente.
